...
<body>
    ...
    <div class="container">
        ...
        <ol>
            <li>Find & Replace text di dokumen Word</li>
            <li>Convert Word ke PDF</li>
            <li><strong>Cari posisi text target di dalam PDF</strong></li>
            <li><strong>Tambahkan gambar berdasarkan offset dari posisi text target</strong></li>
            <li>Gambar akan menumpuk di atas teks/element apapun pada posisi relatif</li>
        </ol>
        <p><strong>Contoh:</strong> Jika text "endah" ditemukan di koordinat (100, 200), dan offset X=10, Y=-15, maka gambar akan ditempatkan di (110, 185) - yaitu 10mm ke kanan dan 15mm ke atas dari text "endah".</p>
...
def add_images_to_pdf(pdf_path, image_rules, target_texts_found):
    """Add images to PDF at positions relative to target text"""
                ...
                if text_positions:
                    ...
                    if os.path.exists(image_path):
                        ...
                        if page_num < len(doc):
                            ...
                            # Calculate image position relative to text position
                            img_x = text_pos['x'] + offset_x_points
                            img_y = text_pos['y'] + offset_y_points
                            ...
                            print(f"Added image for '{target_text}' at relative position ({rule.get('offset_x', 0)}mm, {rule.get('offset_y', -10)}mm) from text")
                else:
...
def add_images_to_pdf_reportlab(pdf_path, image_rules, target_texts_found):
    """Fallback method: Add images using reportlab overlay with text positioning"""
    try:
        from reportlab.pdfgen import canvas
        from reportlab.lib.pagesizes import A4
        from reportlab.lib.units import mm
