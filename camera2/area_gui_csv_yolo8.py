...
class PeopleCounterApp:
    def __init__(self, window, window_title):
        ...
        # YOLO initialization
        self.model = YOLO('yolov8s.pt')  # Menggunakan YOLOv8s
        ...
        # Size Filter
        ttk.Label(control_frame, text="Size Filter (%):").grid(row=3, column=0, padx=5, pady=2)
        self.size_filter_var = tk.DoubleVar(value=50)
        ttk.Scale(control_frame, from_=1, to=100, variable=self.size_filter_var, orient="horizontal").grid(row=3, column=1, padx=5, pady=2)
        ...
        ttk.Label(control_frame, textvariable=self.count_var, font=("", 12, "bold")).grid(row=5, column=0, columnspan=2, pady=5)
