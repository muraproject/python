...
class FireSmokeDetectionGUI:
    def __init__(self, window, window_title):
        ...
        # Video source
        self.video_source = 'api4.mp4'  # Ganti dengan sumber video Anda
            ...
            if self.is_running:
                ...
                if current_time - self.last_detection_time >= self.interval.get():
                    ...
                    if self.fire_detected:
                        self.fire_status.configure(text=f"Fire: Detected ({best_fire_conf:.2f})", foreground='red')
                    else:
                        self.fire_status.configure(text="Fire: Not Detected", foreground='black')
                        
                    if self.smoke_detected:
                        self.smoke_status.configure(text=f"Smoke: Detected ({best_smoke_conf:.2f})", foreground='red')
                    else:
