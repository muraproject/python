...
class HornSimulatorGUI:
    def __init__(self, root):
        ...
        # Create default beep sounds
        self.horn_sounds = []
        for i in range(6):
            try:
                sound = pygame.mixer.Sound(f'horn{i+1}.wav')
            except:
                # Buat beep sederhana
                sample_rate = 44100
                duration = 0.1  # seconds
                frequency = 440 + (i * 100)  # berbeda untuk setiap horn
                samples = int(duration * sample_rate)
                
                # Buat stereo sound (2 channels)
                sound_data = []
                for x in range(samples):
                    value = 32767 * math.sin(2 * math.pi * frequency * x / sample_rate)
                    sound_data.append((value, value))  # Left and right channels
                
                sound = pygame.mixer.Sound(buffer=bytes([int(min(max(v, -32767), 32767)) & 0xFF for t in sound_data for v in t]))
