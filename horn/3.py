...
class HornSimulatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Horn MIDI Simulator")
        ...
        # Initialize Pygame mixer with smaller buffer
        pygame.mixer.init(44100, -16, 2, 512)
        pygame.init()

        # Generate simple beep sounds for each horn
        self.horn_sounds = []
        frequencies = [440, 494, 523, 587, 659, 698]  # A4 to F5
    ...
    def generate_beep(self, frequency, duration=0.1):
        ...
        # Create stereo sound and ensure it's C-contiguous
        stereo = np.ascontiguousarray(np.vstack((scaled, scaled)).T)
    ...
    def process_serial_data(self, line):
        if "Horn" in line:
            ...
            if match:
                ...
                if state == "ON":
                    self.horn_sounds[horn_num].play(fade_ms=5)
                else:
    ...
    def set_tempo(self, bpm):
        if self.ser and self.connected:
            ...
            self.log(f"Tempo set to {bpm} BPM")
