...
class ParkingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🚗 Sistem Parkir - Enhanced with Photo Display")
        self.root.geometry("1500x900")  # Diperbesar untuk menampung foto
    ...
    def create_masuk_tab(self):
        ...
        # Camera display - UKURAN DIPERBESAR dengan kontrol
        camera_control_frame = tk.Frame(left_frame)
        camera_control_frame.pack(fill='x', pady=5)
        
        # Fullscreen button
        self.fullscreen_btn = tk.Button(camera_control_frame, text="🔍 Fullscreen", 
                                      command=self.toggle_camera_fullscreen, bg='#FF9800', fg='white',
                                      font=('Arial', 9))
        self.fullscreen_btn.pack(side='left', padx=5)
        
        # Size toggle button
        self.size_toggle_btn = tk.Button(camera_control_frame, text="📐 Large View", 
                                       command=self.toggle_camera_size, bg='#9C27B0', fg='white',
                                       font=('Arial', 9))
        self.size_toggle_btn.pack(side='left', padx=5)
        
        self.camera_masuk_label = tk.Label(left_frame, bg='gray', text='📷 Camera Starting...', 
                                          width=80, height=30, font=('Arial', 12))
        self.camera_masuk_label.pack(pady=10, fill='both', expand=True)
        
        # Camera size state
        self.camera_size_large = True
    ...
    def create_keluar_tab(self):
        # Main frame - menggunakan PanedWindow untuk layout yang lebih fleksibel
        main_paned = ttk.PanedWindow(self.tab_keluar, orient='horizontal')
        main_paned.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Left side - Barcode input and vehicle details
        left_frame = ttk.LabelFrame(main_paned, text="🔍 Vehicle Search & Details", padding="10")
        ...
        tk.Label(input_frame, text="Masukkan Barcode:", font=('Arial', 10, 'bold')).pack(anchor='w')
        self.barcode_entry = tk.Entry(input_frame, font=('Arial', 12), width=20)
        self.barcode_entry.pack(pady=5, fill='x')
        self.barcode_entry.bind('<Return>', lambda e: self.cek_barcode_async())
        
        button_frame = tk.Frame(input_frame)
        button_frame.pack(fill='x', pady=5)
        
        self.check_btn = tk.Button(button_frame, text="🔍 Cek Detail", command=self.cek_barcode_async,
                                 bg='#2196F3', fg='white', font=('Arial', 10, 'bold'))
        self.check_btn.pack(side='left', padx=(0, 5))
        
        self.exit_btn = tk.Button(button_frame, text="🚪 Buka Pintu Keluar", 
        ...
        # Photo display section - FITUR BARU!
        photo_frame = ttk.LabelFrame(left_frame, text="📸 Vehicle Photos", padding="10")
        photo_frame.pack(fill='both', expand=True, pady=10)
        
        # Photos container dengan scroll
        photos_container = tk.Frame(photo_frame)
        photos_container.pack(fill='both', expand=True)
        
        # Entry photo
        entry_photo_frame = tk.Frame(photos_container)
        entry_photo_frame.pack(side='left', fill='both', expand=True, padx=(0, 5))
        
        tk.Label(entry_photo_frame, text="📷 Foto Masuk", font=('Arial', 10, 'bold')).pack()
        self.entry_photo_label = tk.Label(entry_photo_frame, bg='lightgray', text='No Photo', 
                                         width=25, height=15, relief='sunken', bd=2)
        self.entry_photo_label.pack(pady=5, fill='both', expand=True)
        
        # Exit photo
        exit_photo_frame = tk.Frame(photos_container)
        exit_photo_frame.pack(side='right', fill='both', expand=True, padx=(5, 0))
        
        tk.Label(exit_photo_frame, text="📷 Foto Keluar", font=('Arial', 10, 'bold')).pack()
        self.exit_photo_label = tk.Label(exit_photo_frame, bg='lightgray', text='No Photo', 
                                        width=25, height=15, relief='sunken', bd=2)
        self.exit_photo_label.pack(pady=5, fill='both', expand=True)
        
        # Right side - Parking list and live camera
        right_paned = ttk.PanedWindow(main_paned, orient='vertical')
        main_paned.add(right_paned, weight=1)
        
        # Parking list
        list_frame = ttk.LabelFrame(right_paned, text="🚗 Kendaraan Sedang Parkir", padding="10")
        right_paned.add(list_frame, weight=1)
        
        # Treeview for parking list
        columns = ('Barcode', 'Tanggal', 'Waktu', 'Durasi')
        self.parking_tree = ttk.Treeview(list_frame, columns=columns, show='headings', height=8)
        ...
        scrollbar = ttk.Scrollbar(tree_scroll_frame, orient='vertical', command=self.parking_tree.yview)
        ...
        # Live camera keluar
        camera_frame = ttk.LabelFrame(right_paned, text="📹 Kamera Keluar (Live)", padding="10")
        right_paned.add(camera_frame, weight=1)
        
        self.camera_keluar_label = tk.Label(camera_frame, bg='gray', text='📷 Camera Starting...', 
                                          width=50, height=20, font=('Arial', 10))
        self.camera_keluar_label.pack(fill='both', expand=True)
        
        self.camera_keluar_status = tk.Label(camera_frame, text="Status: Initializing...", 
                                           font=('Arial', 9), fg='orange')
    ...
    def load_and_display_photo(self, photo_path, label_widget, max_width=200, max_height=150):
        """Load and display photo in label widget dengan error handling"""
        try:
            ...
            label_widget.configure(image="", text="📷 Error\nLoading Photo")
    
    def clear_photo_displays(self):
        """Clear both photo displays"""
        self.entry_photo_label.configure(image="", text="📷 No Photo")
        self.entry_photo_label.image = None
        self.exit_photo_label.configure(image="", text="📷 No Photo")
        self.exit_photo_label.image = None
    ...
    def start_background_processes(self):
        ...
        def camera_display_worker():
            while self.running:
                try:
                    # Update masuk camera display - RESOLUSI DINAMIS
                    camera_masuk = self.parking_system.get_camera('masuk')
                    frame = camera_masuk.get_latest_frame()
                    if frame is not None:
                        try:
                            # Get dynamic resize dimensions
                            width, height = self.get_camera_resize_dimensions('masuk')
                    ...
                    # Update keluar camera display - RESOLUSI DINAMIS
                    camera_keluar = self.parking_system.get_camera('keluar')
                    frame = camera_keluar.get_latest_frame()
                    if frame is not None:
                        try:
                            # Get dynamic resize dimensions
                            width, height = self.get_camera_resize_dimensions('keluar')
                            frame_resized = cv2.resize(frame, (width, height))
                            frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
                            img = Image.fromarray(frame_rgb)
                            photo = ImageTk.PhotoImage(image=img)
    ...
    def cek_barcode_async(self):
        """Async barcode check dengan photo display - ENHANCED!"""
        def check_worker():
            try:
                ...
                if not vehicle:
                    def update_ui():
                        ...
                        self.clear_photo_displays()
                    self.root.after_idle(update_ui)
                    return
                
                # Calculate current duration jika kendaraan masih parkir
                if vehicle['status'] == 'masuk':
                    ...
                    exit_info = ""
                    exit_btn_state = 'normal'
                else:
                    # Vehicle sudah keluar
                    duration_str = vehicle.get('durasi', 'Unknown')
                    exit_info = f"\nTanggal Keluar: {vehicle.get('tanggal_keluar', 'N/A')}\n" \
                              f"Waktu Keluar: {vehicle.get('waktu_keluar', 'N/A')}"
                    exit_btn_state = 'disabled'
                
                def update_ui():
                    detail_text = f"✅ DETAIL KENDARAAN\n\n" \
                                 f"Barcode: {vehicle['barcode']}\n" \
                                 f"Status: {vehicle['status'].upper()}\n" \
                                 f"Tanggal Masuk: {vehicle['tanggal_masuk']}\n" \
                                 f"Waktu Masuk: {vehicle['waktu_masuk']}\n" \
                                 f"Durasi: {duration_str}{exit_info}"
                    
                    self.detail_label.configure(text=detail_text)
                    self.detail_frame.pack(fill='x', pady=10)
                    self.detail_label.pack(padx=10, pady=10)
                    
                    self.exit_btn.configure(state=exit_btn_state)
                    self.current_barcode = barcode
                    
                    # LOAD DAN DISPLAY FOTO-FOTO!
                    print(f"Loading entry photo: {vehicle.get('foto_masuk', 'None')}")
                    self.load_and_display_photo(vehicle.get('foto_masuk'), self.entry_photo_label)
                    
                    if vehicle['status'] == 'keluar' and 'foto_keluar' in vehicle:
                        print(f"Loading exit photo: {vehicle.get('foto_keluar', 'None')}")
                        self.load_and_display_photo(vehicle.get('foto_keluar'), self.exit_photo_label)
                    else:
                        # Clear exit photo if vehicle hasn't exited yet
                        self.exit_photo_label.configure(image="", text="📷 Belum Keluar")
                        self.exit_photo_label.image = None
                    
                    if vehicle['status'] == 'masuk':
                        messagebox.showinfo("Sukses", "✅ Kendaraan ditemukan! Masih di dalam parkir.")
                    else:
                        messagebox.showinfo("Info", "ℹ️ Kendaraan ditemukan! Sudah keluar dari parkir.")
            ...
            try:
                ...
                def update_ui():
                    if vehicle:
                        ...
                        # Update photo display with exit photo
                        self.load_and_display_photo(vehicle.get('foto_keluar'), self.exit_photo_label)
                        ...
                        self.root.after(3000, self.clear_photo_displays)
    ...
    def toggle_camera_size(self):
        """Toggle camera display size"""
        try:
            self.camera_size_large = not self.camera_size_large
            if self.camera_size_large:
                self.size_toggle_btn.configure(text="📐 Small View")
            else:
                self.size_toggle_btn.configure(text="📐 Large View")
        except Exception as e:
            print(f"Size toggle error: {e}")
    
    def get_camera_resize_dimensions(self, camera_type):
        """Get appropriate resize dimensions based on camera size setting"""
        if camera_type == 'masuk':
            if self.camera_size_large:
                return (640, 480)  # Large view
            else:
                return (320, 240)  # Small view
        else:  # keluar
            if self.camera_size_large:
                return (400, 300)  # Adjusted for new layout
            else:
                return (200, 150)  # Small view
    ...
    def setup_keyboard_shortcuts(self):
        ...
        try:
            ...
            self.root.bind('<F11>', lambda e: self.toggle_camera_fullscreen())
            
            # Escape to exit fullscreen
            self.root.bind('<Escape>', lambda e: self.root.attributes('-fullscreen', False) or 
                          self.fullscreen_btn.configure(text="🔍 Fullscreen"))
            
            # Ctrl+1 for size toggle
            self.root.bind('<Control-1>', lambda e: self.toggle_camera_size())
...
if __name__ == "__main__":
    print("🚗 Starting Enhanced Parking System with Photo Display...")
    print("📁 Data saved to: parking_data.json & settings.json")
    print("📸 Photos saved to: photos/ folder")
    print("🖼️  NEW FEATURE: Photo display when searching barcodes!")
    ...
    print("   Ctrl+1: Toggle Camera Size")
    print("🎥 Camera Controls: Fullscreen & Size Toggle buttons available")
    print("📷 Photo Features:")
    print("   - Entry photos displayed when searching")
    print("   - Exit photos shown after vehicle exits")
    print("   - Automatic photo scaling and error handling")
