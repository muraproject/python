...
class ParkingApp:
    ...
    def create_history_page(self):
        ...
        self.parked_tree.bind('<Double-1>', self.show_parked_vehicle_details)
        ...
        # Double click to view details
        self.exited_tree.bind('<Double-1>', self.show_exited_vehicle_details)
    ...
    def show_parked_vehicle_details(self, event):
        """Show detailed view for parked vehicle"""
        try:
            ...
            vehicle = self.parking_system.find_vehicle(barcode)
            if not vehicle:
                messagebox.showerror("Error", "Data kendaraan tidak ditemukan!")
                return
            
            # Create detail window
            detail_window = tk.Toplevel(self.root)
            detail_window.title(f"Detail Kendaraan - {barcode}")
            ...
            # Title
            title_label = tk.Label(detail_window, text="DETAIL KENDARAAN SEDANG PARKIR", 
                                 font=('Arial', 16, 'bold'), bg='#E3F2FD')
            title_label.pack(pady=10)
            
            # Main content frame
            content_frame = tk.Frame(detail_window, bg='#E3F2FD')
            content_frame.pack(fill='both', expand=True, padx=20, pady=10)
            
            # Left side - Details
            details_frame = tk.LabelFrame(content_frame, text="Informasi Kendaraan", 
                                        font=('Arial', 12, 'bold'), bg='#E3F2FD')
            ...
            base_cost = hours * self.parking_system.settings.get('tarif_per_jam', 2000)
            if vehicle.get('is_member', False):
                estimated_cost = int(base_cost * 0.8)
            else:
            ...
            details_data = [
                ...
                ("RFID ID:", vehicle.get('rfid_id', '-')),
                ("Plat Nomor:", vehicle.get('plat_nomor', '-')),
                ("Tanggal Masuk:", vehicle['tanggal_masuk']),
                ("Waktu Masuk:", vehicle['waktu_masuk']),
                ("Durasi Parkir:", duration_str),
                ("Status:", "MEMBER" if vehicle.get('is_member', False) else "REGULAR"),
                ("Estimasi Biaya:", f"Rp.{estimated_cost:,},-")
            ]
            
            for i, (label, value) in enumerate(details_data):
                tk.Label(details_frame, text=label, font=('Arial', 10, 'bold'), 
                        bg='#E3F2FD', anchor='w').grid(row=i, column=0, sticky='w', padx=10, pady=5)
                tk.Label(details_frame, text=str(value), font=('Arial', 10), 
                        bg='white', relief='sunken', anchor='w', width=25).grid(row=i, column=1, padx=10, pady=5)
            
            # Right side - Entry Photo
            photo_frame = tk.LabelFrame(content_frame, text="Foto Masuk", 
                                      font=('Arial', 12, 'bold'), bg='#E3F2FD')
            photo_frame.pack(side='right', fill='both', expand=True)
            
            photo_label = tk.Label(photo_frame, bg='lightgray', text='Loading Photo...', 
                                 width=40, height=20)
            photo_label.pack(fill='both', expand=True, padx=10, pady=10)
            
            # Load and display entry photo
            self.load_and_display_photo(vehicle.get('foto_masuk'), photo_label, 350, 250)
            
            # Close button
            close_btn = tk.Button(detail_window, text="TUTUP", command=detail_window.destroy,
                                bg='#9E9E9E', fg='white', font=('Arial', 12, 'bold'))
            close_btn.pack(pady=10)
            
        except Exception as e:
            print(f"Show parked vehicle details error: {e}")
            messagebox.showerror("Error", f"Error menampilkan detail: {str(e)}")
    
    def show_exited_vehicle_details(self, event):
        """Show detailed view for exited vehicle with both photos"""
        try:
            selection = self.exited_tree.selection()
            if not selection:
                return
                
            item = self.exited_tree.item(selection[0])
            barcode = item['values'][0]
            
            # Find vehicle data
            vehicle = None
            for v in self.parking_system.parking_data:
            ...
            # Create detail window (wider for two photos)
            detail_window = tk.Toplevel(self.root)
            detail_window.title(f"Detail Kendaraan Keluar - {barcode}")
            ...
            # Title
            title_label = tk.Label(detail_window, text="DETAIL KENDARAAN SUDAH KELUAR", 
                                 font=('Arial', 16, 'bold'), bg='#E3F2FD')
            title_label.pack(pady=10)
            ...
            details_data = [
                ("Barcode:", vehicle['barcode'], "RFID ID:", vehicle.get('rfid_id', '-')),
                ("Plat Masuk:", vehicle.get('plat_nomor', '-'), "Plat Keluar:", vehicle.get('nopol_keluar', '-')),
                ("Tanggal Masuk:", vehicle['tanggal_masuk'], "Tanggal Keluar:", vehicle.get('tanggal_keluar', '-')),
                ("Waktu Masuk:", vehicle['waktu_masuk'], "Waktu Keluar:", vehicle.get('waktu_keluar', '-')),
                ("Durasi Parkir:", vehicle.get('durasi', '-'), "Total Biaya:", f"Rp.{vehicle.get('biaya', 0):,},-"),
                ("Status Member:", "YA" if vehicle.get('is_member', False) else "TIDAK", "", "")
            ]
            
            for i, (label1, value1, label2, value2) in enumerate(details_data):
                # Left column
                tk.Label(details_frame, text=label1, font=('Arial', 10, 'bold'), 
                        bg='#E3F2FD', anchor='w').grid(row=i, column=0, sticky='w', padx=10, pady=3)
                tk.Label(details_frame, text=str(value1), font=('Arial', 10), 
                        bg='white', relief='sunken', anchor='w', width=20).grid(row=i, column=1, padx=10, pady=3)
                
                # Right column (if data exists)
                if label2:
                    tk.Label(details_frame, text=label2, font=('Arial', 10, 'bold'), 
                            bg='#E3F2FD', anchor='w').grid(row=i, column=2, sticky='w', padx=10, pady=3)
                    tk.Label(details_frame, text=str(value2), font=('Arial', 10), 
                            bg='white', relief='sunken', anchor='w', width=20).grid(row=i, column=3, padx=10, pady=3)
            
            # Bottom - Photos
            photos_frame = tk.Frame(content_frame, bg='#E3F2FD')
            photos_frame.pack(fill='both', expand=True)
            
            # Entry Photo
            entry_photo_frame = tk.LabelFrame(photos_frame, text="Foto Masuk", 
                                            font=('Arial', 12, 'bold'), bg='#E3F2FD')
            entry_photo_frame.pack(side='left', fill='both', expand=True, padx=(0, 5))
            
            entry_photo_label = tk.Label(entry_photo_frame, bg='lightgray', text='Loading Entry Photo...', 
                                       width=40, height=15)
            entry_photo_label.pack(fill='both', expand=True, padx=10, pady=10)
            
            # Exit Photo
            exit_photo_frame = tk.LabelFrame(photos_frame, text="Foto Keluar", 
                                           font=('Arial', 12, 'bold'), bg='#E3F2FD')
            exit_photo_frame.pack(side='right', fill='both', expand=True, padx=(5, 0))
            
            exit_photo_label = tk.Label(exit_photo_frame, bg='lightgray', text='Loading Exit Photo...', 
                                      width=40, height=15)
            exit_photo_label.pack(fill='both', expand=True, padx=10, pady=10)
            
            # Load and display photos
            self.load_and_display_photo(vehicle.get('foto_masuk'), entry_photo_label, 400, 230)
            self.load_and_display_photo(vehicle.get('foto_keluar'), exit_photo_label, 400, 230)
            ...
            print(f"Show exited vehicle details error: {e}")
            messagebox.showerror("Error", f"Error menampilkan detail: {str(e)}")
