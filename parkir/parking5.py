...
class ParkingApp:
    ...
    def create_general_settings(self, parent):
        ...
        tk.Label(settings_frame, text="Tarif per Hari (Rp):", 
               font=('Arial', 10)).grid(row=0, column=0, sticky='w', padx=10, pady=10)
        self.tarif_entry = tk.Entry(settings_frame, font=('Arial', 10), width=20)
        self.tarif_entry.grid(row=0, column=1, padx=10, pady=10)
        self.tarif_entry.insert(0, str(self.parking_system.settings.get('tarif_per_hari', 5000)))
    ...
    def process_search_result(self, vehicle, search_key, search_type):
        ...
        def search_result_worker():
            try:
                ...
                # Calculate cost based on days (minimum 1 day)
                days = max(1, int(duration.total_seconds() / 86400))  # 86400 seconds in a day
                if duration.total_seconds() % 86400 > 0:  # If there's remaining time, add 1 day
                    days += 1
                
                base_cost = days * self.parking_system.settings.get('tarif_per_hari', 5000)
                if vehicle.get('is_member', False):
                ...
                def update_ui():
                    ...
                    if search_type == 'rfid' and vehicle.get('is_member', False):
                        ...
                        messagebox.showinfo("Sukses", f"Member {member_name} ditemukan via RFID!\nDays: {days}, Discount: 20%")
                    else:
                        messagebox.showinfo("Sukses", f"Kendaraan ditemukan! Days: {days}")
            ...
            try:
                ...
                def update_ui():
                    if vehicle:
                        ...
                        self.clear_exit_displays()
                        self.clear_search_fields()  # Clear both search fields
                        self.current_barcode = None
                    else:
    ...
    def save_general_settings(self):
        try:
            ...
            self.parking_system.settings['tarif_per_hari'] = tarif
            self.parking_system.save_data()
    ...
    def start_background_processes(self):
        ...
        def camera_display_worker():
            while self.running:
                try:
                    # Update entry camera with 16:9 aspect ratio
                    camera_masuk = self.parking_system.get_camera('masuk')
                    frame = camera_masuk.get_latest_frame()
                    if frame is not None:
                        try:
                            # Use 16:9 aspect ratio (640x360 for better video display)
                    ...
                    # Update exit camera with 16:9 aspect ratio
