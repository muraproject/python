...
# MODIFIKASI: DataManager yang hanya menggunakan person sebagai motor
class NonBlockingDataManager:
    ...
    def initialize_counts(self):
        """Inisialisasi counts - HANYA menggunakan person untuk motor, hapus motorcycle"""
        base_counts = {f'{direction}{i}': 0 for direction in ['up', 'down'] for i in range(1, 7)}
        return {
            'car': base_counts.copy(),
            'person': base_counts.copy(),  # person akan diperlakukan sebagai motor
    ...
    def get_summary_counts(self):
        """MODIFIKASI: Menggunakan person dan bicycle sebagai motor"""
        ...
        # MODIFIKASI: Gabungkan person dan bicycle sebagai motor
        ...
        person_motor_up = max(person_up, bicycle_up)  # Gabungkan person dan bicycle
        ...
        person_motor_down = max(person_down, bicycle_down)  # Gabungkan person dan bicycle
...
def run_vehicle_counter(settings_manager, output_dir='output', yolo_model='yolov8s.pt'):
    """Main function dengan deteksi person sebagai motor"""
    ...
    window_name = "Vehicle Counter - Motor Detection"
    ...
    logger.info("Starting main loop - Person detected as Motor...")
    ...
    try:
        while running:
                    ...
                    for r in results:
                        ...
                        if boxes is not None:
                            for box in boxes:
                                ...
                                # MODIFIKASI: Hanya deteksi car, person, truck, bus, bicycle (HAPUS motorcycle)
                                if conf > confidence_threshold and class_name in ['car', 'person', 'truck', 'bus', 'bicycle']:
                                    ...
                                    # MODIFIKASI: Ubah label display - person dan bicycle jadi "motor"
                                    ...
                                    detected_objects.append({
                                        'id': matched_id,
                                        'class': class_name,  # class asli untuk counting
                                        'display_class': display_class,  # class untuk display
                                        'box': (x1, y1, x2, y2),
            ...
            for obj in detected_objects:
                ...
                # MODIFIKASI: Draw label menggunakan display_class (motor untuk person/bicycle)
                direction = obj.get('direction', '')
                direction_color = obj.get('direction_color', color)
                display_name = obj['display_class'][:3]  # Ambil 3 karakter pertama
                label = f"{display_name} {direction}".strip()
            ...
            if show_sidebar:
                ...
                y_pos += 20
                # MODIFIKASI: Label di sidebar tetap "Motor" untuk person_motor
                cv2.putText(sidebar, f"Motor: {counts['person_motor_up']}↑ {counts['person_motor_down']}↓", (15, y_pos), font, 0.5, (200, 200, 200), 1)
                ...
                y_pos += 20
                cv2.putText(sidebar, "Person -> Motor", (15, y_pos), font, 0.4, (0, 255, 255), 1)
                y_pos += 20
                ...
                cv2.putText(display_frame, "Person -> Motor", (10, 90), font, 0.5, (0, 255, 255), 2)
...
if __name__ == "__main__":
   parser = argparse.ArgumentParser(description='Vehicle Counter - Person as Motor')
   ...
   logger.info("Starting Vehicle Counter - Person Detection as Motor")
   logger.info("Person and Bicycle will be detected and labeled as 'Motor'")
   logger.info("Motorcycle detection has been removed")
