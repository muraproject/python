...
# Optimasi untuk Intel CPU
def optimize_for_intel():
    """Optimize OpenCV and other libraries for Intel processors"""
    try:
        # Enable Intel optimizations untuk OpenCV
        os.environ['OPENCV_DNN_BACKEND'] = 'OPENCV'  # Gunakan OpenCV DNN backend
        
        # Set thread optimization untuk Intel CPU
        if 'Xeon' in platform.processor():
            # Set jumlah thread berdasarkan core prosesor
            physical_cores = os.cpu_count() // 2  # Estimasi physical cores
            ...
            logger.info(f"Optimized for Intel Xeon with {physical_cores} threads")
            
            # Attempt to enable IPP (Intel Performance Primitives)
            try:
                cv2.setUseIPP(True)
                ipp_status = "enabled" if cv2.useIPP() else "not available"
                logger.info(f"Intel IPP: {ipp_status}")
            except:
                pass
                
            # Try to use Intel MKL optimizations if available
            try:
                os.environ['MKL_DEBUG_CPU_TYPE'] = '5'  # Force Intel optimized code paths
                os.environ['MKL_ENABLE_INSTRUCTIONS'] = 'AVX2'  # Use AVX2 if available
                logger.info("MKL optimizations applied")
            except:
                pass
            
            return True
        else:
            logger.info("Non-Xeon processor detected")
            # Still optimize threads for other CPUs
            logical_cores = os.cpu_count()
            cv2.setNumThreads(logical_cores)
            return False
            
    except Exception as e:
        logger.error(f"Error during Intel optimization: {e}")
        return False
...
class SettingsManager:
    def __init__(self, settings_file="config/settings_opencv.json"):
        ...
        self.default_settings = {
            ...
            'api_url': "http://192.168.1.5:5000",  # URL server API
            ...
            'processing': {
                ...
                'render_thread': True,  # Gunakan render thread terpisah
                'intel_optimization': True  # Gunakan optimasi Intel
            },
            'display': {
...
class DataManager:
    ...
    def save_current_counts(self):
        ...
        try:
            ...
            if success:
                logger.info(f"Successfully saved counts to server: {counts}")
    ...
    def update_camera_source(self):
        ...
        Hanya melakukan update jika ada perubahan pada sumber kamera
        """
        try:
            # Force refresh camera list
            cameras = self.get_cameras()
            if not cameras:
                # Hanya update jika video_source ada nilainya (perlu dihapus)
                if self.settings_manager.settings.get('video_source') is not None:
                    ...
                    logger.warning("No cameras available from API, cleared video source")
                    return True  # Return True untuk menandakan perubahan
                return False  # Tidak ada kamera, tidak ada perubahan
            ...
            if camera:
                ...
                # Periksa apakah ada perubahan pada sumber kamera
                if (previous_settings['video_source'] == camera['ip'] and
                    previous_settings['camera_name'] == camera['name'] and
                    previous_settings['camera_mode'] == camera['mode']):
                    # Tidak ada perubahan, tidak perlu update
                    logger.debug("No changes in camera settings, skipping update")
                    return False
                
                # Ada perubahan, update settings
        ...
        while not self.stopped:
            if not self.grabbed:
                ...
                if consecutive_errors > max_consecutive_errors:
                    logger.error("Too many consecutive frame errors, attempting reconnection")
                    try:
                        self.cap.release()
                        time.sleep(2)
            ...
            if elapsed >= self.frame_interval:
                ...
                consecutive_errors = 0  # Reset error counter on successful grab
...
class TrafficAnalytics:
    ...
    def update_count(self, object_type, direction, line_num):
        ...
        direction_key = f"{direction}{line_num}"
        if object_type in self.counts and direction_key in self.counts[object_type]:
            self.counts[object_type][direction_key] += 1
            self.interval_counts[object_type][direction_key] += 1
...
def run_vehicle_counter(
    ...
    if settings_manager.settings['processing']['intel_optimization']:
        optimize_for_intel()
    
    # Buat directory output jika belum ada
    os.makedirs(output_dir, exist_ok=True)
    
    # Inisialisasi data manager dan analytics
    data_manager = DataManager(settings_manager)
    analytics = TrafficAnalytics(num_lines=6, interval_minutes=1)
    ...
    use_render_thread = settings_manager.settings['processing']['render_thread']
    ...
    # Initialize render queue for separate render thread
    render_queue = queue.Queue(maxsize=5)
    render_thread_active = False
    render_thread = None
    render_latest_frame = None
    key_press_queue = queue.Queue()
    
    # Render thread function
    def render_thread_func():
        """Separate thread for rendering frames to the screen"""
        nonlocal render_latest_frame
        try:
            logger.info("Render thread started")
            while render_thread_active:
                try:
                    # Get frame from queue
                    frame = render_queue.get(timeout=0.5)
                    if frame is None:
                        render_queue.task_done()
                        continue
                        
                    # Store latest frame for screenshot
                    render_latest_frame = frame.copy()
                    
                    # Display frame
                    cv2.imshow(window_name, frame)
                    
                    # Get key press with short timeout
                    key = cv2.waitKey(1) & 0xFF
                    if key != 255:  # If a key was pressed
                        key_press_queue.put(key)
                        
                    render_queue.task_done()
                except queue.Empty:
                    # Just check for key press if queue is empty
                    key = cv2.waitKey(1) & 0xFF
                    if key != 255:  # If a key was pressed
                        key_press_queue.put(key)
                except Exception as e:
    ...
    while running:
        ...
        try:
            while not video_capture.stopped and running:
                ...
                # Check for key presses in the render thread
                if use_render_thread:
                    try:
                        while not key_press_queue.empty():
                            ...
                            if key in key_actions:
                                ...
                                if action == 'quit':
                                    ...
                                    if render_latest_frame is not None:
                                        screenshot_mgr.maybe_save(render_latest_frame, 0)  # Force screenshot
                ...
                # Check if analytics interval completed
                analytics.check_interval()
                
                # Get latest frame
                frame = video_capture.read()
                if frame is None:
                    logger.warning("Null frame received, waiting...")
                    time.sleep(0.1)
                    continue
                ...
                if show_sidebar:
                    ...
                    cv2.putText(sidebar, f"Analytics ({analytics.interval_minutes}min):", (10, y_pos), 
                               font, font_scale_small, (200, 200, 200), 1)
                    y_pos += 20
                ...
                # Display final frame - either directly or via render thread
                if use_render_thread:
                    try:
                        # Put frame in render queue (non-blocking)
                        if not render_queue.full():
                            render_queue.put(display_frame.copy(), block=False)
                    except queue.Full:
                        # Skip frame if queue is full
                        pass
                else:
                    # Display directly if not using render thread
            ...
            if use_render_thread and render_thread_active:
                render_thread_active = False
