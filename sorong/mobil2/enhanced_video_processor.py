...
class VideoDisplay:
    ...
    """
    def __init__(self, width=800, height=600, resize_factor=0.8):
        self.display_width = width
        self.display_height = height
        self.resize_factor = resize_factor
        logger.info(f"Initialized VideoDisplay with size {width}x{height}, resize factor: {resize_factor}")
        
    def set_resize_factor(self, factor):
        """Update resize factor"""
        self.resize_factor = factor
        logger.info(f"Updated resize factor to {factor}")
        
    def update_display_size(self, width, height):
        """Update display size"""
        self.display_width = width
        self.display_height = height
        logger.info(f"Updated display size to {width}x{height}")
...
class EnhancedStreamReader:
    ...
    """
    def __init__(self, video_source, buffer_size=30, target_fps=30.0, resize_factor=0.8):
    ...
    def _initialize_capture(self):
        ...
        try:
            ...
            # Calculate dimensions using resize factor
            ...
            logger.info(f"Resized dimensions: {self.width}x{self.height} (factor: {self.resize_factor})")
            ...
            self.original_width = 800  # Default values if initialization fails
    ...
    def update_resize_factor(self, new_factor):
        ...
        if new_factor <= 0 or new_factor > 2.0:
            logger.error(f"Invalid resize factor: {new_factor}")
            return False
    ...
    def get_original_dimensions(self):
        """Get original frame dimensions from source"""
...
class EnhancedVideoProcessor:
    ...
    def __init__(self, settings_manager, data_manager):
        try:
            ...
            # Get settings
            self.target_fps = self.settings_manager.settings['processing']['target_fps']
            self.confidence_threshold = self.settings_manager.settings['processing']['confidence_threshold']
            self.tracking_points = self.settings_manager.settings['processing']['tracking_points']
            
            # Get resize factor for display
            self.resize_factor = self.settings_manager.settings['display']['resize_factor']
            ...
            # Set max trajectory points
            if hasattr(self.tracker, 'max_points'):
                self.tracker.max_points = self.tracking_points
            
            # Setup monitoring and display
            self.monitor = Monitor(data_manager)
            self.video_display = VideoDisplay(
                width=self.settings_manager.get_display_size()[0],
                height=self.settings_manager.get_display_size()[1],
            ...
            logger.info(f"Enhanced VideoProcessor initialized with: FPS={self.target_fps}, Confidence={self.confidence_threshold}, Resize={self.resize_factor}")
    ...
    def initialize_video_capture(self):
        ...
        try:
            ...
            # Create and start enhanced reader with resize factor
            ...
            # Get original dimensions from reader
            original_width, original_height = reader.get_original_dimensions()
            
            # Update settings manager with original dimensions
            self.settings_manager.update_display_size(original_width, original_height)
            
            # Update display size based on resize factor
            width, height = reader.get_dimensions()
            self.video_display.update_display_size(width, height)
        ...
        try:
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    if conf > self.confidence_threshold and class_name in ['car', 'person', 'truck', 'bus', 'bicycle', 'motorcycle']:
    ...
    def update_settings(self, target_fps=None, confidence=None, resize_factor=None):
        """Update processor settings"""
        if target_fps is not None:
            self.target_fps = target_fps
            self.settings_manager.settings['processing']['target_fps'] = target_fps
            logger.info(f"Updated target FPS to {target_fps}")
        ...
        if resize_factor is not None:
            self.resize_factor = resize_factor
            self.settings_manager.settings['display']['resize_factor'] = resize_factor
            self.video_display.set_resize_factor(resize_factor)
            logger.info(f"Updated resize factor to {resize_factor}")
