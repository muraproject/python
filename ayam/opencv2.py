...
class HSVColorTracker:
    ...
    def setup_trackbars(self):
        ...
        cv2.resizeWindow('HSV Controls', 600, 500)
    ...
    def update_info_display(self, coverage_percent):
        """Update info display di HSV Controls window"""
        # Buat canvas untuk info
        info_canvas = np.ones((500, 600, 3), dtype=np.uint8) * 50  # Dark gray background
        
        # Counter besar dengan warna merah
        counter_text = f"COUNTER: {self.counter}"
        cv2.putText(info_canvas, counter_text, (50, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 4)
        
        # Point status
        point_status = "GREEN" if self.point_is_green else "RED"
        point_color = (0, 255, 0) if self.point_is_green else (0, 0, 255)
        cv2.putText(info_canvas, f"POINT: {point_status}", (50, 110), cv2.FONT_HERSHEY_SIMPLEX, 1.0, point_color, 3)
        
        # HSV Range info
        info_lines = [
            f"HSV Range:",
            f"  H: {self.lower_h} - {self.upper_h}",
            f"  S: {self.lower_s} - {self.upper_s}", 
            f"  V: {self.lower_v} - {self.upper_v}",
            f"",
            f"Tracked Area: {coverage_percent:.1f}%",
            f"",
            f"Click Point: {self.click_point if self.click_point else 'Not Set'}",
            f"",
            f"Instructions:",
            f"• Click on video to set tracking point",
            f"• RED point = in white mask area",
            f"• GREEN point = in black mask area", 
            f"• Counter +1 when RED -> GREEN",
            f"• Press 'c' to reset counter"
        ]
        
        # Tambahkan info lines
        for i, line in enumerate(info_lines):
            y_pos = 160 + i * 25
            if y_pos < 480:  # Pastikan tidak keluar dari canvas
                cv2.putText(info_canvas, line, (50, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        # Tampilkan di window HSV Controls
        cv2.imshow('HSV Controls', info_canvas)
    ...
    def run(self, video_folder="videos"):
        ...
        for video_path in video_files:
            ...
            while True:
                ...
                # Update info display di HSV Controls window
                self.update_info_display(coverage_percent)
                
                # Siapkan frame untuk display (tanpa text overlay)
