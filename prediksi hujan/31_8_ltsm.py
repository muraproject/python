...
def train_lstm(X_train, y_train, epochs=50, batch_size=32, patience=10):
...
def plot_predictions(y_true, y_pred):
    ...
    plt.title('Actual vs Predicted Rainfall (LSTM)')
    plt.savefig('lstm_actual_vs_predicted.png')
    plt.close()

def save_models(lstm_model, scaler, lstm_path='lstm_model', scaler_path='scaler.joblib'):
    try:
        model_json = lstm_model.to_json()
        with open(f"{lstm_path}_architecture.json", "w") as json_file:
            json_file.write(model_json)
        lstm_model.save_weights(f"{lstm_path}.weights.h5")
        
        joblib.dump(scaler, scaler_path)
        print("Model dan scaler berhasil disimpan.")
    except Exception as e:
        print(f"Terjadi kesalahan saat menyimpan model: {str(e)}")

def load_models(lstm_path='lstm_model', scaler_path='scaler.joblib'):
    try:
        with open(f"{lstm_path}_architecture.json", "r") as json_file:
...
def main():
    choice = input("Apakah Anda ingin melakukan training baru? (y/n): ").lower()
    ...
    if choice == 'y':
        ...
        lstm_model, history = train_lstm(X_train_scaled, y_train)
        
        plot_history(history)

        save_models(lstm_model, scaler)
    else:
        lstm_model, scaler = load_models()
        if lstm_model is None or scaler is None:
            print("Gagal memuat model. Keluar dari program.")
            return
    ...
    print(f"\nModel Evaluation (with threshold {threshold} mm):")
    ...
    print(f"\nAkurasi klasifikasi hujan/tidak hujan: {accuracy:.4f}")
